...
describe ActivitiesHelper do
  ...
  it "should have the right message for a wall comment" do
    ...
    feed_message(activity).should =~ Regexp.new(I18n.t('person.person_wall',
                                     :person => I18n.t('person.persons',
                                     :person => person.name)))
  ...
  it "should have the right message for a photo" do
    ...
    photo = Photo.new({:uploaded_data => @image, :owner => @current_person, :gallery => @gallery})
    photo.save
